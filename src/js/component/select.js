...
window.addEventListener('mousedown', function (event) {
  select.forEach(function (e) {
    if (event.target !== e.querySelector('.select_btn') && event.target.closest('.select.open') === null) {
      e.classList.remove('open');
      e.querySelector('.select_opt').dispatchEvent(new CustomEvent('close', { detail: { closeOther: document.querySelectorAll('.select.open').length > 0 } }))
    }
  })
})
...
select.forEach(function (e) {
  var selectBtn = e.querySelector('.select_btn');
  var selectOpt = e.querySelectorAll('.select_opt li .option');
...
function toggleSelect(e) {
  ...
  var option = this.parentNode.querySelector('.select_opt'),
  ...
  if (!this.parentNode.classList.contains('open')) {
    ...
    option.addEventListener('close', closeCustomDrop, { once: true });
    window.addEventListener('scroll', closeCustomDrop.bind(option), { once: true });
    if (document.querySelector('.wrap')) document.querySelector('.wrap').addEventListener('scroll', closeCustomDrop, { once: true });
  }
}
...
function setVal() {
  var text = this.innerHTML,
    val = this.value,
    btn = this.closest('.select').querySelector('.select_btn'),
    option = this.closest('.select_opt');
